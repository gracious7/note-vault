...
const Subjects: React.FC<SubAndBranch> = ({ semester, branch }) => {
  ...
  return (
    <>
      {subject ? (
        <BookSection subject={subject} semester={semester} branch={branch} />
      ) : (
        <main className="dashboard full-screen m-12">
          ...
          <h2 className="text-lg md:text-2xl font-bold text-center m-2  ">
            Semester: {semester}
          </h2>
      ...
      {subject && (
        <div className="relative h-[4rem] z-40">
          <Button
            className="bg-blue-500 hover:bg-blue-600 w-auto self-center rounded-md p-3 text-white font-bold outline-none active:shadow-lg transition-all absolute top-0 right-[50%] translate-x-1/2 mt-[5rem] ripple"
            onClick={gotoSub}
          >
            Back to Subject
          </Button>
